function [] = save2hdf5(savepath, chunksz, input_data, label_data, deinterlaced_data, interlaced_data, inv_mask_data)
    ...
    for batchno = 1:floor(count/chunksz)
        ...
        if exist('interlaced_data','var')
            ...
            curr_dat_sz = write_hdf5(savepath, chunksz, ~created_flag, startloc, b_input_data, b_label_data, b_deinterlaced_data, b_interlaced_data, b_inv_mask_data);
        else
            startloc = struct('input_data', [1, 1, 1, totalct+1], ...
                              'label_data', [1, 1, 1, totalct+1], ...
                              'deinterlaced_data', [1,1,1,totalct+1]);
            curr_dat_sz = write_hdf5(savepath, chunksz, ~created_flag, startloc, b_input_data, b_label_data, b_deinterlaced_data); 
        end
...
function [curr_dat_sz, curr_lab_sz] = write_hdf5(filename, chunksz, create, startloc, input_data, label_data, deinterlaced_data, interlaced_data, inv_mask_data) 
  ...
  % chunksz (used only in create mode), specifies number of samples to be stored per chunk (see HDF5 documentation on chunking) for creating HDF5 files with unbounded maximum size - TLDR; higher chunk sizes allow faster read-write operations 
    ...
    if create
        ...
        if ~exist('startloc','var')
            ...
            prev_deinterlaced_data_sz = info.Datasets(3).Dataspace.Size;
            ...
            if exist('interlaced_data','var')
                prev_interlaced_data_sz = info.Datasets(4).Dataspace.Size;
